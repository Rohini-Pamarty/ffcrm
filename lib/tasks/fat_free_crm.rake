...
namespace :crm do
  ...
  namespace :setup do
    ...
    task :admin => :environment do
      username, password, email, phone = ENV["USERNAME"], ENV["PASSWORD"], ENV["EMAIL"], ENV["PHONE"]
      unless username && password && email && phone
        ...
        loop do
          ...
          print "\nPhoneNumber [#{phone}]: "
          reply = STDIN.gets.strip
          phone = reply unless reply.blank?
      ...
      user.update_attributes(:username => username, :password => password, :email => email, :phone => phone)
